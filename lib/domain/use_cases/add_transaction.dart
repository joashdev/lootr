...
class AddTransaction {
  ...
  Future<Result<String>> call(Transaction transaction) async {
    ...
    if (transaction.direction != 'expense' &&
      ...
      return Failure('Invalid transaction direction: ${transaction.direction}',
          code: 'invalid_direction');
    }

    try {
      final account =
          await _accountRepo.watchById(transaction.accountId).first;
      if (account == null) {
        return Failure('Account not found: ${transaction.accountId}',
            code: 'account_not_found');
      }
      if (account.deletedAt != null) {
        return Failure('Account is deleted: ${transaction.accountId}',
            code: 'account_deleted');
      }
      if (account.isArchived) {
        return Failure('Account is archived: ${transaction.accountId}',
