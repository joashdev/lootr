...
class EditTransaction {
  ...
  Future<Result<void>> call(Transaction updated) async {
    ...
    if (updated.direction != 'expense' && updated.direction != 'income') {
      return Failure(
        'Invalid transaction direction: ${updated.direction}',
    ...
    try {
      ...
      if (original == null) {
        return Failure(
          'Transaction not found: ${updated.id}',
          code: 'not_found',
        );
      }
      if (original.deletedAt != null) {
        return Failure(
          'Cannot edit a deleted transaction: ${updated.id}',
          code: 'transaction_deleted',
        );
      }

      if (updated.accountId != original.accountId) {
        final account = await _accountRepo.watchById(updated.accountId).first;
        if (account == null) {
          return Failure(
            'Account not found: ${updated.accountId}',
            code: 'account_not_found',
          );
        }
        if (account.deletedAt != null) {
          return Failure(
            'Account is deleted: ${updated.accountId}',
            code: 'account_deleted',
          );
        }
        if (account.isArchived) {
          return Failure(
            'Account is archived: ${updated.accountId}',
      ...
      return Failure(
        'Failed to load original transaction: $e',
        code: 'load_error',
      );
    }
