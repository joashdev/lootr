...
class CreateTransfer {
  ...
  Future<Result<String>> call(Transfer transfer) async {
    if (transfer.sourceAccountId == transfer.destinationAccountId) {
      return Failure('Source and destination accounts must be different',
          code: 'same_account');
    }
    ...
    try {
      final source =
          await _accountRepo.watchById(transfer.sourceAccountId).first;
      if (source == null) {
        return Failure('Source account not found: ${transfer.sourceAccountId}',
