...
1. Update `pubspec.yaml`, for example `0.1.0-alpha.1+1`.
2. Merge the release-ready commit into `main`.
3. Confirm Flutter CI passes on `main`.
4. Create and push the matching tag:

   ```sh
   git tag -s v0.1.0-alpha.1 -m "Lootr v0.1.0-alpha.1"
...
```sh
sha256sum -c lootr-v0.1.0-alpha.1-android.apk.sha256
gh attestation verify lootr-v0.1.0-alpha.1-android.apk \
  --repo joashdev/lootr
```
